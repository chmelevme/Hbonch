...
def done(id):
    ...
    if Deadline_status.query.filter_by(deadline_id=deadline.id, user_id=current_user.id).first().status != 2:
        return redirect(url_for('main.main_route'))
    Deadline_status.query.filter_by(deadline_id=deadline.id,user_id=current_user.id).first().status = 1
